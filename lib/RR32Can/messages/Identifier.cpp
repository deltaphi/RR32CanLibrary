...
unsigned long Identifier::makeIdentifier() const {
  ...
  packetId = static_cast<unsigned long>(prio_) << (16 + 1 + 8);
  packetId |= static_cast<unsigned long>(command_) << (16 + 1);
  packetId |= static_cast<unsigned long>(response_) << (16);
  packetId |= hash_;
...
void Identifier::printAll() const {
  printf("Prio: %#04X, Command: %#04X, Response: %#04X, Hash: %#06X", prio_, command_, response_, hash_);
}
