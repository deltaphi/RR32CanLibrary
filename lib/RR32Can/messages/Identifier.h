...
class Identifier {
 public:
  ...
  uint8_t prio_ = 0;  // 4 bit

  /// The Marklin command
  uint8_t command_ = 0;

  /// Whether this is a response packet
  bool response_ = false;  // 1 bit

  /// The hash of the packet
  uint16_t hash_ = 0;

  Identifier() = default;
  Identifier(uint8_t command, uint16_t hash) : command_(command), hash_(hash) {}

  bool operator==(const Identifier& other) const {
    return prio_ == other.prio_ && command_ == other.command_ && response_ == other.response_ && hash_ == other.hash_;
  }
   ...
   */
  void computeAndSetHash(uint32_t deviceUuid) { hash_ = computeSenderHash(deviceUuid); }
